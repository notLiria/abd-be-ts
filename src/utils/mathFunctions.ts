...
export const stringifyDfcEqn = (expCoeffs: DfcCoeffs) => {
  return `f(x) = ${expCoeffs.p0 / expCoeffs.lambda0} * e^(${
    expCoeffs.lambda0
  }x) + ${expCoeffs.p1 / expCoeffs.lambda1} * e^(${expCoeffs.lambda1} * x) + ${
    expCoeffs.c
  }`;
};

export const stringifyDfcDerivativeEqn = (expCoeffs: DfcCoeffs) => {
  return `f(x) = ${expCoeffs.p0} * e^(${expCoeffs.lambda0}x) + ${expCoeffs.p1} * e^(${expCoeffs.lambda1}x)`;
};
