...
export const curveFit = (func: FunctionToOptimize, xVals: number[], yVals: number[], initialParams: number[], learningRate = 0.01, maxIterations = 1000, tolerance = 1e-6): OptimizationResult  => {
    ...
    for (let iter = 0; iter < maxIterations; iter++) {
        ...
        for (let i = 0; i < xVals.length; i++) {
            ...
            for (let j = 0; j < params.length; j++) {
                ...
                const derivative = (tweakedResult - func(xVals[i], ...params)) / tolerance;
