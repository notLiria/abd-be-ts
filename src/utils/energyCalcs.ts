...
export const exponentialDfFunc = (x: number, coeffs: DfcCoeffs) => {
  return (
    (coeffs.p0 / coeffs.lambda0) * math.exp(coeffs.lambda0 * x) +
    (coeffs.p1 / coeffs.lambda1) * math.exp(coeffs.lambda1 * x) +
    coeffs.c
  );
};
...
export const calcEnergyAtPoint = (
  ...
  const yVals = xVals.map(xVal => {
    return exponentialDfFunc(xVal, coeffs);
  });
...
export const getLongbowPoint = (
  dfData: Point[],
  regressionCoeffs: DfcCoeffs,
): number | null => {
  const minX = Math.min(...dfData.map(point => point.x));
  ...
  const calcLineAreaAtPoint = (point: Point) => {
    ...
    const p0 = {
      x: dfData[0].x,
      y: exponentialDfFunc(dfData[0].x, regressionCoeffs),
    };
  ...
  const xValues = Array.from(
    {length: Math.ceil((maxX - minX) / step)},
    (_, idx) => minX + idx * step,
  );
  ...
  const longbowPoints = xValues.map((pointCandidate: number) => {
    const lineEndPoint: Point = {
      x: pointCandidate,
      y: exponentialDfFunc(pointCandidate, regressionCoeffs),
    };

    const areaUnderExpCurve = calcEnergyAtPoint(
      regressionCoeffs,
      dfData,
      pointCandidate,
    );
