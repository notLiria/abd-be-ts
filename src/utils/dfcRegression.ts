...
import {exponentialDfFunc} from './energyCalcs';
...
export const calcCentralDifferences = (dfData: Point[]): Point[] => {
  ...
  const coeffsStart = fitPoly(
    dfData.slice(0, boundaryDegree + 1),
    boundaryDegree,
  );
...
export const exponentiallyFitDfData = (dfData: Point[]) => {
  ...
  const output: any = {};
  const xVals: number[] = dfData.map(point => {
    return point.x;
  });
  const yVals: number[] = dfData.map(point => {
    return point.y;
  });
  const centralDiffs: number[] = calcCentralDifferences(dfData).map(
    point => point.y,
  );
  // Calculate integrals
  ...
  const X = new Matrix(
    columnStack(
      xVals.map(x => {
        return math.exp(lambdas[0] * x);
      }),
      xVals.map(x => {
        return math.exp(lambdas[1] * x);
      }),
    ),
  );
  ...
  const dfFunc: curveFit.FunctionToOptimize = (x: number, c: number) => {
    return (
      (output.p0 / lambdas[0]) * math.exp(lambdas[0] * x) +
      (output.p1 / lambdas[1]) * math.exp(lambdas[1] * x) +
      c
    );
  };
...
export const calcRegressionCurve = (dfData: Point[], coeffs: DfcCoeffs) => {
  return dfData.map((point: Point) => {
    return {x: point.x, y: exponentialDfFunc(point.x, coeffs)};
  });
};

export const calcDerivative = (dfData: Point[], coeffs: DfcCoeffs) => {
  return dfData.map((point: Point) => {
    return {
      x: point.x,
      y:
        coeffs.p0 * math.exp(coeffs.lambda0 * point.x) +
        coeffs.p1 * math.exp(coeffs.lambda1 * point.x),
    };
  });
};
